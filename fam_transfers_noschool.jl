...
function Vncj_solve(vncjp1, wncjp1, Vj_nc, PFj_nc, model, j)
    (; beta, gamma,a_grid_nocollege, tasks_idx_nc) = model

    fill!(Vj_nc, 0f0)
    fill!(PFj_nc, 0f0)

    # When creating interpolation objects for Vj+1:
    vnc_itp = LinearInterpolation((a_grid_nocollege, z_grid[R]), vncjp1[R, m, n, t, :, :, :, :], extrapolation_bc=Flat())
    ...
    for n_next in 1:6, t_next in fam_type, m_next in marital_status
        ...
        for i_zp1 in 1:zpnts
            ...
            for shock_in_next in 1:2, shock_out_next in 1:2, past_in_next in 1:2, past_out_next in 1:2
                ...
                # expected value contribution from next period state
                expected_value += prob_family_transition * pi_z * shock_out_next_prob *  shock_in_next_prob* vnc_itp[R, m_next, n_next, t_next, shock_in, shock_out, past_in_next, past_out_next](ap1, i_zp1)
...
function Wncj(wncjp1, Wj_nc, WPFj_nc, model, j)
    (; survival_risk, beta, gamma , shock_resources, a_grid_nocollege, tasks_idx_nc, jpnts) = model
...
function EWnc_jp1(model, wncjp1_itp, j, R, m, n, ap1, i_z, shock_in, shock_out, past_in, past_out)
    ...
    for shock_in_next in 1:2, shock_out_next in 1:2, past_in_next in 1:2, past_out_next in 1:2
        ...
        # Expected value contribution from next period state
        expected_value += shock_out_next_prob * shock_in_next_prob* wncjp1_itp[shock_in, shock_out, past_in_next, past_out_next](ap1, i_z)
