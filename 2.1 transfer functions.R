...
black_probit_out = glm(Provided_Support_Indicator ~ log_nonasset_income + log_asset_income + Age + Age2 + Family_Unit_Size + Head_College +
...
black_probit_in = glm(Received_Support_Indicator ~ log_nonasset_income + log_asset_income  + Age + Age2 + Family_Unit_Size + Head_College +
                        Marital_Status + family_type + received_transfer_past + provided_transfer_past + Birth_Cohort + Year_bins,
                      family = binomial(link = "probit"),  
                      data = psid_clean %>% filter(Race_Head == "Black")%>% filter(!is.na(provided_transfer_past)))
summary(black_probit_in)

white_probit_in = glm(Received_Support_Indicator ~ log_nonasset_income + log_asset_income  + Age + Age2 + Family_Unit_Size + Head_College +
                        Marital_Status + family_type + received_transfer_past + provided_transfer_past + Birth_Cohort +  Year_bins,
                      family = binomial(link = "probit"), 
                      data = psid_clean %>% filter(Race_Head == "White")%>% filter(!is.na(provided_transfer_past)))
summary(white_probit_in)

white_probit_out = glm(Provided_Support_Indicator ~log_nonasset_income + log_asset_income + Age + Age2 + Family_Unit_Size + Head_College +
                         Marital_Status + family_type + received_transfer_past + provided_transfer_past + Birth_Cohort +  Year_bins,
                       family = binomial(link = "probit"), 
                       data = psid_clean %>% filter(Race_Head == "White")%>% filter(!is.na(provided_transfer_past)))
...
black_transfer_out = lm(log_transfer_out ~ log_nonasset_income + log_asset_income + Age + Age2 + Family_Unit_Size + Head_College+
...
black_transfer_in = lm(log_transfer_in ~ log_nonasset_income + log_asset_income + Age + Age2 + Family_Unit_Size  + Head_College +
                         Marital_Status + family_type  + Year_bins,
                      data = psid_clean %>% filter(Race_Head == "Black") %>%  filter(Received_Support_Indicator == 1))
summary(black_transfer_in)

white_transfer_out = lm(log_transfer_out ~ log_nonasset_income + log_asset_income + Age + Age2 + Family_Unit_Size + Head_College +
                          Marital_Status + family_type  + Year_bins,
                       data = psid_clean %>% filter(Race_Head == "White") %>%  filter(Provided_Support_Indicator == 1))
summary(white_transfer_out)

white_transfer_in = lm(log_transfer_in ~ log_nonasset_income + log_asset_income + Age + Age2 + Family_Unit_Size + Head_College +
                         Marital_Status + family_type + Year_bins,
