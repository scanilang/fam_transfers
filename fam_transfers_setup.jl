...
###############################################################################################
function compute_borrowing_limit(z_grid, r_loan, jpnts, R, e, phi, working_years)
    z_min  = z_grid[R][1]
    j_grad = e == 2 ? 3 : 5
    ...
    for j in 1:jpnts
        if j <= working_years
            ...
            d_limit[j] = 0.0   # no borrowing in retirement
        end
    end
...
# Load shock table
if pwd() == "/Users/scanilang/Documents/econ/umn/family_transfers/2026"
    fam_shock_df = CSV.read("/Users/scanilang/Documents/econ/umn/family_transfers/data/family_shock_table.csv", DataFrame)
else
    fam_shock_df = CSV.read("/users/4/canil007/family_transfers/Data/family_shock_table.csv", DataFrame)
end
